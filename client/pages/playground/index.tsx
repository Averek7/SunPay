...
const Page: React.FC = () => {
  ...
  const testAddWorker = async () => {
    ...
    try {
      ...
      setTestData((prev) => ({
        ...prev,
        selectedWorkerPda: workerPda.toBase58(),
      }));
  ...
  const testWithdraw = async () => {
    ...
    try {
      ...
      addLog(
        `Withdrawing ${testData.withdrawAmount} SOL from treasury`,
        "info",
      );
  ...
  const testFetchUserOrgs = async () => {
    ...
    try {
      ...
      orgs.forEach((org, i) => {
        addLog(
          `${i + 1}. ${org.name} - Treasury: ${org.treasury} SOL - Workers: ${org.workersCount}`,
          "info",
        );
      });
    } catch (error) {
  ...
  const testFetchOrgWorkers = async () => {
    ...
    try {
      ...
      const workers = await fetchOrganizationWorkers(
        program,
        testData.selectedOrgPda,
      );
  ...
  const testFetchWorkerDetails = async () => {
    ...
    try {
      ...
      const worker = await fetchWorkerDetails(
        program,
        testData.selectedWorkerPda,
      );

      addLog(`Salary: ${worker.salary} SOL`, "success");
      addLog(
        `Last Paid: ${new Date(worker.lastPaidCycle).toLocaleString()}`,
        "info",
      );
    } catch (error) {
  ...
  const testCheckPayrollDue = async () => {
    ...
    try {
      ...
      const result = await checkPayrollDue(
        program,
        testData.selectedOrgPda,
        "monthly",
      );
      ...
      if (result.due) {
        addLog(
          `Payroll is DUE! ${result.workers.length} worker(s) need payment`,
          "success",
        );
      } else {
  ...
  const testGetOrgBalance = async () => {
    ...
    try {
      ...
      const balance = await getOrganizationBalance(
        program,
        testData.selectedOrgPda,
      );
  ...
  const testCalculatePayrollCost = async () => {
    ...
    try {
      ...
      const cost = await calculateTotalPayrollCost(
        program,
        testData.selectedOrgPda,
      );
  ...
  const TestButton = ({
    ...
    const variants = {
      write:
        "bg-linear-to-r from-[#DC1FFF] to-[#00FFA3] hover:from-[#00FFA3] hover:to-[#DC1FFF] text-black",
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-black via-slate-900 to-black relative overflow-hidden pt-20">
      ...
      <div
        className="absolute bottom-20 right-10 w-96 h-96 bg-[#00FFA3]/10 rounded-full blur-[120px] animate-pulse"
                  ...
                  {[
                    ...
                    <div key={field.key}>
                      ...
                      <div className="relative group/input">
                        ...
                        />
                        {(field.key === "selectedOrgPda" ||
                          field.key === "selectedWorkerPda") &&
                          testData[field.key as keyof TestData] && (
                            <button
                              onClick={() =>
                                copyToClipboard(
                                  testData[field.key as keyof TestData],
              ...
              <div className="relative z-10">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <TestButton
                    ...
                    loading={loading === "addWorker"}
                    variant="write"
                    label="2. Add Worker"
                  />
                  <TestButton
                    onClick={testFundTreasury}
                    loading={loading === "fundTreasury"}
                    variant="write"
                    label="3. Fund Treasury"
                  />
                  <TestButton
                    onClick={testProcessPayroll}
                    loading={loading === "processPayroll"}
                    variant="write"
                    label="4. Process Payroll"
                  />
                  <TestButton
                    onClick={testWithdraw}
                    loading={loading === "withdraw"}
                    variant="write"
                    label="5. Withdraw Funds"
                  />
                </div>
              </div>
            </div>
              ...
              <div className="relative z-10">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <TestButton
                    ...
                    loading={loading === "fetchUserOrgs"}
                    variant="read"
                    label="6. My Orgs"
                  />
                  <TestButton
                    onClick={testFetchAllOrgs}
                    loading={loading === "fetchAllOrgs"}
                    variant="read"
                    label="7. All Orgs"
                  />
                  <TestButton
                    onClick={testFetchOrgDetails}
                    loading={loading === "fetchOrgDetails"}
                    variant="read"
                    label="8. Org Details"
                  />
                  <TestButton
                    onClick={testFetchOrgWorkers}
                    loading={loading === "fetchOrgWorkers"}
                    variant="read"
                    label="9. Org Workers"
                  />
                  <TestButton
                    onClick={testFetchWorkerDetails}
                    loading={loading === "fetchWorkerDetails"}
                    variant="read"
                    label="10. Worker Details"
                  />
                  <TestButton
                    onClick={testFetchWorkersByWallet}
                    loading={loading === "fetchWorkersByWallet"}
                    variant="read"
                    label="11. My Workers"
                  />
                  <TestButton
                    onClick={testCheckPayrollDue}
                    loading={loading === "checkPayrollDue"}
                    variant="read"
                    label="12. Payroll Due?"
                  />
                  <TestButton
                    onClick={testGetOrgBalance}
                    loading={loading === "getOrgBalance"}
                    variant="read"
                    label="13. Org Balance"
                  />
                  <TestButton
                    onClick={testCalculatePayrollCost}
                    loading={loading === "calculatePayrollCost"}
                    variant="read"
                    label="14. Payroll Cost"
                  />
                </div>
              </div>
            </div>
