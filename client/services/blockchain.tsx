...
import type { PayrollProgram } from "@/contracts/target/types/payroll_program";
import idlJson from "@/contracts/target/idl/payroll_program.json";
import {
  Connection,
  PublicKey,
  SystemProgram,
...
export const fetchOrganizationDetails = async (
  ...
  const org = (await program.account.organization.fetch(
    new PublicKey(orgPda),
  )) as RawOrganization;
...
export const fetchWorkerDetails = async (
  ...
  const worker = (await program.account.worker.fetch(
    new PublicKey(workerPda),
  )) as RawWorker;
 ...
 */
const serializeOrganizations = (
  organizations: { publicKey: PublicKey; account: RawOrganization }[],
): Organization[] => {
...
const serializeWorkers = (
  workers: { publicKey: PublicKey; account: RawWorker }[],
): Worker[] => {
  return workers
    .map((w) => ({
