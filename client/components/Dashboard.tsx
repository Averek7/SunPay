        ...
        if (def.typeName === "ZodObject" && def.shape) {
          ...
          Object.entries(shape).forEach(([key, zodType]) => {
            ...
            properties[key] = {
              type,
              description:
                actualDef.description ||
                innerDef.description ||
                `${key} parameter`,
            };
      ...
      try {
        ...
        if (
          ...
          if (result.success && Array.isArray(result.organizations)) {
            const mappedOrgs: PayrollSummary[] = result.organizations.map(
              (org: unknown) => {
                const orgData = org as Record<string, unknown>;
    ...
    try {
      ...
      while (iterations < maxIterations) {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `AI API failed (${response.status}): ${errorData.error?.message || "Unknown error"}`,
          );
        }
        ...
        if (message.tool_calls && message.tool_calls.length > 0) {
          for (const toolCall of message.tool_calls) {
            ...
            try {
              const tool =
                blockchainMcpTools[toolName as keyof typeof blockchainMcpTools];
              if (!tool || !tool.execute) {
              ...
              if (
                ...
                Symbol.asyncIterator in toolOutput
              ) {
                let str = "";
